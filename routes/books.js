...
router.findBookByLike = (req,res)=>{
    let likes = req.params.like;
    res.setHeader('Content-Type','application/json');
    //find all books that their likes  are bigger or equal to requested value and must be integer
    if(likes>=0 && Math.round(likes) == likes) {
        Book.find({"like": {"$gte": likes}}, function (err, book) {
            if (err || book.length == 0)
        ...
        res.send({message: 'Input number only allows integers and must not be less than 0'});
